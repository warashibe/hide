...
export const downloadJson = async ({ fn, get, set, global: { db, _db } }) => {
  const date = Date.now()
  const fileName = `hide_${get("local_data_id") || "local"}_${date}.json`
...
export const importJson = async ({
  ...
  await lf.getItem("local_data_id", json.id || null)
  await lf.getItem("backup", json.backup || null)
